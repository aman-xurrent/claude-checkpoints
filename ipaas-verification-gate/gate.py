...
Subcommands
  ...
  finalize    Phase-aware finish check: shape of the diff for the phase, rubocop,
              yarn check and lint, specs and proof (phases 6 and 7), references.
              Exit 0 pass, 1 a gate failed, 2 the diff has the wrong shape.
...
# Generated by `rake js:routes:typescript`, not checked in; yarn check fails without them.
FRONTEND_ROUTES_FILE_NAMES = ("routes.js", "routes.ts", "routes.d.ts")
...
def javascript_checks(root, paths):
    ...
    checks = []
    if not any((platform / "app/javascript" / name).exists() for name in FRONTEND_ROUTES_FILE_NAMES):
        checks.append(command_check("routes", ["bundle", "exec", "rake", "js:routes:typescript"], platform,
                                    environment=rails_boot_environment(root)))
    return checks + [command_check("yarn check", ["yarn", "check"], platform), command_check("yarn lint", ["yarn", "lint"], platform)]


def rails_boot_environment(root):
    """Enough environment to boot Rails in a worktree that has no slot: the test environment, plus the
    main clone's local env files when this worktree lacks its own. Nothing here touches a database."""
    environment = toolchain_environment()
    environment["RAILS_ENV"] = "test"
    if not (root / TEST_ENVIRONMENT_FILE).exists():
        for name in (TEST_ENVIRONMENT_FILE.name, *SHARED_ENVIRONMENT_FILE_NAMES):
            for key, value in environment_file_values(MAIN_REPOSITORY / "platform" / name).items():
...
def spec_checks(root, paths):
    checks = [command_check(f"rspec {project}", ["bundle", "exec", "rspec", *files], root / project)
...
def finalize(arguments):
    ...
    checks += rubocop_checks(root, paths)
    checks += javascript_checks(root, paths)
    if phase in (6, 7):
        checks += spec_checks(root, paths)
        checks.append(proof_check(root, paths))
    else:
        checks.append(("specs", "skip", f"phase {phase} does not run specs"))
