...
| `phased/` | The phase loop: 7-phase development as the default Claude Code workflow for ipaas, advanced by the author's GitHub approval per phase and run by a small Python daemon. Steps A and B built: skills linked locally into every worktree, finalize in a slotted checks worktree, Phase trailer, and the daemon that starts the next phase on the approver's review; see `phased/README.md` and `phased/docs/plan.md`. |
